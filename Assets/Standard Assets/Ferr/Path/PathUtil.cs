...
namespace Ferr {
	public static class PathUtil {
        ...
        public static Vector2 GetSegmentNormal(int i, List<Vector2> aPath, bool aClosed) {
			...
			if (count < 2) return Vector2.up;
			Vector2 dir;
			if (!aClosed && i == count - 1) {
				dir = aPath[WrapIndex(i, count, aClosed)] - aPath[WrapIndex(i - 1, count, aClosed)];
			} else {
				dir = aPath[WrapIndex(i + 1, count, aClosed)] - aPath[WrapIndex(i, count, aClosed)];
			}

			float mag = dir.magnitude;
			return new Vector2(dir.y/mag, -dir.x/mag);
		}
