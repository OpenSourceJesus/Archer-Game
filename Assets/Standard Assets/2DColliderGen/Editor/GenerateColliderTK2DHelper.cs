...
public class GenerateColliderTK2DHelper {
    ...
    protected List<IslandDetector.Region> mIslands = null;
    protected List<IslandDetector.Region> mSeaRegions = null;

	protected ColliderRegionData[] mColliderRegions = null;

	Texture2D mMainTex = null;
    bool[] mBinaryImage = null;
    int mBinaryImageWidth;
    int mBinaryImageHeight;
    int[] mClassificationImage = null;
	...
	public bool CalculateUnreducedOutline() {
		...
		CalculateUnreducedOutlineForAllColliderRegions(ref mColliderRegions, ref mOutlineAlgorithm, mRegionIndependentParameters, mColliderRegionParameters, mBinaryImage, mBinaryImageWidth, mBinaryImageHeight, ccwVertexOrder);
		return true;
	}
	...
	public static void SetupColliderRegionParameters(ref ColliderRegionParametersTK2D[] colliderRegionParameters, int defaultMaxPointCount,
		...
		if (shallResetRegionParameters) {
			...
			for (int seaRegionIndex = 0; seaRegionIndex < seaRegions.Count; ++seaRegionIndex) {
	...
	public static int ReduceOutlineForAllColliderRegions(ref ColliderRegionData[] colliderRegions, ref PolygonOutlineFromImageFrontend outlineAlgorithm,
		...
		for (int count = 0; count < colliderRegions.Length; ++count) {
			...
			outlineAlgorithm.ReduceOutline(colliderRegions[count].mIntermediateOutlineVertices, ref colliderRegions[count].mReducedOutlineVertices, ref colliderRegions[count].mUsedIndices, colliderRegions[count].mOutlineVertexOrderIsCCW);
